...
type RegistrationMethod = "madrid" | "single";

type MethodCountrySelection = {
...
function buildMethodCountrySelection(input: {
  country_id?: string | null;
  country_ids?: string[];
  registration_methods?: RegistrationMethod[];
  madrid_country_ids?: string[];
  ...
  if (
    Array.isArray(input.madrid_country_ids) ||
    Array.isArray(input.single_country_ids)
  ) {
    return {
      madrid_country_ids: uniqueStrings(input.madrid_country_ids ?? []),
      single_country_ids: uniqueStrings(input.single_country_ids ?? []),
    };
  }

  const countryIds = uniqueStrings(
    input.country_ids?.length
      ? input.country_ids
      : input.country_id
        ? [input.country_id]
        : [],
  );
  const methods = input.registration_methods?.length
    ? input.registration_methods
    : ["single"];
...
function selectedRegistrationMethods(
  selection: MethodCountrySelection,
): RegistrationMethod[] {
  const methods: RegistrationMethod[] = [];
  if (selection.madrid_country_ids.length > 0) {
    methods.push("madrid");
  }
  if (selection.single_country_ids.length > 0) {
    methods.push("single");
  }
  return methods;
}

function uniqueCountryIds(selection: MethodCountrySelection): string[] {
...
function calculateLegacyLines(
  ...
  fee_item: string;
  amount_cny_cents: number;
  source_pricing_entry_id: string;
  ...
  for (const countryId of countryIds) {
    ...
    lines.push(
      ...matched.map((entry) => ({
        fee_item: entry.fee_item,
        amount_cny_cents: entry.amount_cny_cents,
        source_pricing_entry_id: entry.id,
        registration_method: registrationMethod,
...
function calculateMethodLines(
  ...
  if (selection.madrid_country_ids.length > 0) {
    ...
    for (const countryId of selection.madrid_country_ids) {
  ...
  if (selection.single_country_ids.length > 0) {
    if (hasSingle) {
      for (const countryId of selection.single_country_ids) {
      ...
      const legacyLines = calculateLegacyLines(
        selection.single_country_ids,
        serviceTier,
        "single",
      );
...
export const defaultHandlers = [
  ...
  http.get("/api/v1/single-class-pricing-entries", ({ request }) => {
    ...
    const items = singleClassPricingEntries.filter(
      (entry) =>
        entry.effective_to == null &&
        (!country || entry.country_id === country),
    );
    return HttpResponse.json({ items });
  }),
  ...
  http.post("/api/v1/quotations", async ({ request }) => {
    ...
    const now = new Date().toISOString();
    const selection = buildMethodCountrySelection(body);
    const countryIds = uniqueCountryIds(selection);
    const registrationMethods = selectedRegistrationMethods(selection);
    const q = {
      id: randomUUID(),
      customer_id: body.customer_id,
      country_id: countryIds[0],
      country_ids: countryIds,
      madrid_country_ids: selection.madrid_country_ids,
      single_country_ids: selection.single_country_ids,
  ...
  http.patch("/api/v1/quotations/:id", async ({ params, request }) => {
    ...
    const selection = buildMethodCountrySelection({
      country_id:
        typeof body.country_id === "string" ? body.country_id : q.country_id,
      country_ids: body.country_ids ?? q.country_ids,
      madrid_country_ids: body.madrid_country_ids ?? q.madrid_country_ids,
      single_country_ids: body.single_country_ids ?? q.single_country_ids,
      registration_methods: body.registration_methods ?? q.registration_methods,
    });
    const countryIds = uniqueCountryIds(selection);
    Object.assign(q, body, {
      country_id: countryIds[0] ?? q.country_id,
      country_ids: countryIds,
      madrid_country_ids: selection.madrid_country_ids,
      single_country_ids: selection.single_country_ids,
      registration_methods: selectedRegistrationMethods(selection),
  ...
  http.post("/api/v1/quotations/:id/submit", ({ params }) => {
    ...
    const selection = buildMethodCountrySelection(q);
    const methodLines = calculateMethodLines(
      selection,
  ...
  http.post("/api/v1/quotations/:id/copy", ({ params }) => {
    ...
    const copied = {
      ...
      madrid_country_ids: [...src.madrid_country_ids],
      single_country_ids: [...src.single_country_ids],
...
export function seedMadridPricingEntry(p: {
  country_id?: string | null;
  sequence_no?: number | null;
  country_area: string;
  official_fee_chf_cents: number;
  ...
  const now = new Date().toISOString();
  madridPricingEntries.push({
    id: randomUUID(),
    country_id: p.country_id ?? null,
    sequence_no: p.sequence_no ?? null,
    country_area: p.country_area,
    official_fee_chf_cents: p.official_fee_chf_cents,
    agency_fee_cny_cents: p.agency_fee_cny_cents,
    is_base_fee: p.is_base_fee,
    notes: null,
    effective_from: now,
    effective_to: null,
    created_by: adminUser.id,
...
export function seedSingleClassPricingEntry(p: {
  country_id: string;
  continent: string;
  country_area: string;
  first_class_fee_cny_cents: number;
...
export function seedQuotationDraft(p: {
  ...
  quotations.push({
    ...
    madrid_country_ids: [],
    single_country_ids: [p.country_id],
    nice_category_codes: [],
